...
module.exports = async (directory, props, functions, att) => {
    return new Promise( async (resolve, reject) => {
        ...
        if(! directory ) {
            ...
            try {
                fetch(`https://fsdb.tk/cli/set`, { method: 'POST', headers: { 'Content-type': 'application/json' }, body: JSON.stringify({ getFrom: 'cli', props: _props, infosDirectory: directory, infosObject: {} })}).then(res => res.json())
                    .then(res => {
                        _nextTick(res)
                    })
